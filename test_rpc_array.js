async function test() {
  ...
  const createRes = await fetch(url + '/rest/v1/rpc/create_profile', {
    ...
    body: JSON.stringify({
      p_player_id: crypto.randomUUID(), p_handle: handle, p_password_hash: 'hash123', p_parent_email: 'test@example.com'
    })
  });
  ...
  console.log('Result begins with [ ?', text.trim().startsWith('['));
  console.log(text);
}
test();
